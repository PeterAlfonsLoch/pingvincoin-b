...
The options known to work for building Pingvincoin Core on Windows are:
...
Note that for WSL the Pingvincoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/pingvincoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
