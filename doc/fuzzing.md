# Fuzzing Pingvincoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Pingvincoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
# libFuzzer" on https://github.com/pingvincoin/pingvincoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Pingvincoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/pingvincoin_report.pdf) includes an external overview of the status of Pingvincoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/pingvincoin/pingvincoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/pingvincoin/pingvincoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/pingvincoin/pingvincoin/tree/master/src/test/fuzz).
...
It is possible to specify `pingvincoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`pingvincoin-core/qa-assets`](https://github.com/pingvincoin-core/qa-assets) repo.

To fuzz `process_message` using the [`pingvincoin-core/qa-assets`](https://github.com/pingvincoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/pingvincoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`pingvincoin-core/qa-assets`](https://github.com/pingvincoin-core/qa-assets) repo.

Every single pull request submitted against the Pingvincoin Core repo is automatically tested against all inputs in the [`pingvincoin-core/qa-assets`](https://github.com/pingvincoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Pingvincoin Core more robust.
...
# Fuzzing Pingvincoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Pingvincoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Pingvincoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Pingvincoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Pingvincoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Pingvincoin
Core without having to write any custom fuzzing harness. The `pingvincoind` server
...
```sh
$ mkdir pingvincoin-honggfuzz-p2p/
$ cd pingvincoin-honggfuzz-p2p/
$ git clone https://github.com/pingvincoin/pingvincoin
$ cd pingvincoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
EOF
$ make -C src/ pingvincoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/pingvincoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Pingvincoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Pingvincoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/pingvincoin/pingvincoin/pull/22472) for more information.
...
Pingvincoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/pingvincoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=pingvincoin-core).
...
because Pingvincoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_pingvincoin-core/latest).
