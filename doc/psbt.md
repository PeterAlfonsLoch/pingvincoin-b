# PSBT Howto for Pingvincoin Core

Since Pingvincoin Core 0.17, an RPC interface exists for Partially Signed Pingvincoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/pingvincoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Pingvincoin transactions that are not fully signed
...
#### Multisig with multiple Pingvincoin Core instances
...
Pingvincoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `pingvincoin-cli` is used.
