Pingvincoin Core version 0.19.0.1 is now available from:

  <https://pingvincoincore.org/bin/pingvincoin-core-0.19.0.1/>
...
Pingvincoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Pingvincoin Core on unsupported systems.
...
to open the Pingvincoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Pingvincoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/pingvincoin/pingvincoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Pingvincoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Pingvincoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `pingvincoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Pingvincoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Pingvincoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Pingvincoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Pingvincoin Core,
  ...
  This prevents Pingvincoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Pingvincoin Core and nodes that have manually enabled BIP37 support,
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Pingvincoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Pingvincoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Pingvincoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Pingvincoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Pingvincoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Pingvincoin AEAD (jonasschnelli)
...
- #15639 pingvincoin-wallet tool: Drop libpingvincoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in pingvincoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "pingvincoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of pingvincoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from pingvincoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_pingvincoin to auto generated project files (sipsorcery)
...
- #15773 Add PingvincoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_pingvincoin output (practicalswift)
...
- #15896 feature_filelock, interface_pingvincoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Pingvincoin Core (luke-jr)
...
- #16277 Suppress output in test_pingvincoin for expected errors (gertjaap)
...
- #16489 log: harmonize pingvincoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (pingvincoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example pingvincoin.conf (fanquake)
...
- #16080 build/doc: update pingvincoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in pingvincoin.conf (fanquake)
...
- #16621 add default pingvincoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the pingvincoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, pingvincoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/pingvincoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/pingvincoin/pingvincoin/).
