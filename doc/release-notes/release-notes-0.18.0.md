Pingvincoin Core version 0.18.0 is now available from:

  <https://pingvincoincore.org/bin/pingvincoin-core-0.18.0/>
...
`/Applications/Pingvincoin-Qt` (on Mac) or `pingvincoind`/`pingvincoin-qt` (on
Linux).
...
recommended to use Pingvincoin Core on unsupported systems.
...
10.10. Additionally, Pingvincoin Core does not yet change appearance when
...
`contrib/init/pingvincoind.service`, it has been changed to use
`/var/lib/pingvincoind` as the data directory instead of
`~pingvincoin/.pingvincoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/pingvincoind` will
exist has enough space (check using `df -h /var/lib/pingvincoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/pingvincoin/pingvincoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://pingvincoincore.org/en/contact/) the Pingvincoin Core
...
The systemd init file (`contrib/init/pingvincoind.service`) has been changed
to use `/var/lib/pingvincoind` as the data directory instead of
`~pingvincoin/.pingvincoin`. This change makes Pingvincoin Core more consistent with
...
`datadir` option in `/etc/pingvincoin/pingvincoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/pingvincoin/pingvincoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/pingvincoin/pingvincoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/pingvincoin/pingvincoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/pingvincoin/pingvincoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/pingvincoin/pingvincoin/blob/master/doc/pingvincoin-conf.md)
  about the `pingvincoin.conf` file describes how to use it to configure
  Pingvincoin Core.

- A new document introduces Pingvincoin Core's BIP174 [Partially-Signed
  Pingvincoin Transactions
  (PSBT)](https://github.com/pingvincoin/pingvincoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/pingvincoin/pingvincoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Pingvincoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Pingvincoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/pingvincoin/pingvincoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/pingvincoin/pingvincoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Pingvincoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Pingvincoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/pingvincoin/pingvincoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/pingvincoin/pingvincoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart pingvincoind with the `-deprecatedrpc=generate`
...
- A new `pingvincoin-wallet` tool is now distributed alongside Pingvincoin
...
This section describes planned changes to Pingvincoin Core that may affect
other Pingvincoin software and services.

- Since version 0.16.0, Pingvincoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Pingvincoin Core 0.20 (expected about
  a year after 0.18), Pingvincoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Pingvincoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Pingvincoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Pingvincoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Pingvincoin Core
...
- A sub-project of Pingvincoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Pingvincoin Core.  See their [project
  page](https://github.com/pingvincoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Pingvincoin Core's own implementation, although entropy
  gathered by Pingvincoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Pingvincoin Core a
...
- On macOS, Pingvincoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `pingvincoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libpingvincoin_server.a` (jnewbery)
...
- #13926 [Tools] pingvincoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like pingvincoind (MarcoFalke)
...
- #14521 qt, docs: Fix `pingvincoin-qt -version` output formatting (hebasto)
...
- #11625 Add PingvincoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libpingvincoin_server` (laanwj)
...
- #15154 Configure: pingvincoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/pingvincoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Pingvincoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libpingvincoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in pingvincoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Pingvincoin Core (Christewart)
...
- #12246 Bugfix: Only run pingvincoin-tx tests when pingvincoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_pingvincoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_pingvincoin` (MarcoFalke)
...
- #14186 pingvincoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_pingvincoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update pingvincoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/pingvincoin-conf.md (hebasto)
...
- #15354 Add missing `pingvincoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct pingvincoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support pingvincoin folder on OSX (JimmyMow)
