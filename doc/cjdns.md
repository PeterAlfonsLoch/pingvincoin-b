# CJDNS support in Pingvincoin Core

It is possible to run Pingvincoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Pingvincoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Pingvincoin network and individual nodes.
...
before it will work with your Pingvincoin Core node. This is described in steps
...
## Run Pingvincoin Core with CJDNS

Once you are connected to the CJDNS network, the following Pingvincoin Core
...
When enabled, this option tells Pingvincoin Core that it is running in an
...
IPv6 local network. This helps Pingvincoin Core perform better address management:
...
CJDNS support was added to Pingvincoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `pingvincoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/pingvincoin/pingvincoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Pingvincoin Core

There are several ways to see your CJDNS address in Pingvincoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `pingvincoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `pingvincoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `pingvincoin-cli help getnodeaddresses` for details.
