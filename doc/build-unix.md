UNIX BUILD NOTES
====================
Some notes on how to build Pingvincoin Core in Unix.
...
memory available when compiling Pingvincoin Core. On systems with less, gcc can be
...
To build Pingvincoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build pingvincoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a pingvincoin-qt executable will be
built by default.
...
...
to: /path/to/pingvincoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/pingvincoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Pingvincoin Core can
