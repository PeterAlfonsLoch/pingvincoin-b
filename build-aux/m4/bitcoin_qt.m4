dnl Copyright (c) 2013-2016 The Pingvincoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set pingvincoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$pingvincoin_qt_want_version" = "auto" && test "$pingvincoin_qt_force" != "yes"; then
    if test "$pingvincoin_enable_qt" != "no"; then
      AC_MSG_WARN([$1; pingvincoin-qt frontend will not be built])
    fi
    pingvincoin_enable_qt=no
    pingvincoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$pingvincoin_enable_qt" != "no" && test "$pingvincoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build pingvincoin-qt GUI (default=auto)])],
    [
     pingvincoin_qt_want_version=$withval
     if test "$pingvincoin_qt_want_version" = "yes"; then
       pingvincoin_qt_force=yes
       pingvincoin_qt_want_version=auto
     fi
    ],
    [pingvincoin_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); pingvincoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: pingvincoin_enable_qt, pingvincoin_enable_qt_dbus, pingvincoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$pingvincoin_cv_static_qt" = "yes"; then
  ...
  BITCOIN_QT_CHECK([
    pingvincoin_enable_qt=yes
    pingvincoin_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      pingvincoin_enable_qt_test=no
    fi
    pingvincoin_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      pingvincoin_enable_qt_dbus=yes
    fi
  ...
  if test $pingvincoin_enable_qt = "yes"; then
    AC_MSG_RESULT([$pingvincoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$pingvincoin_enable_qt])
  fi
...
dnl Output: pingvincoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, pingvincoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [pingvincoin_cv_static_qt=yes],
      [pingvincoin_cv_static_qt=no])
    ])
])
