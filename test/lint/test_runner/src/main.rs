...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from pingvincoin-config.h is used without the header being included",
...
fn get_pathspecs_exclude_whitespace() -> Vec<String> {
    ...
    list.extend(
        [
            ...
            "contrib/init/pingvincoind.openrc",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./src/config/pingvincoin-config.h.in";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args([
                    // These are exceptions which don't use pingvincoin-config.h, rather the Makefile.am adds
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <config/pingvincoin-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
^^^
One or more files use a symbol declared in the pingvincoin-config.h header. However, they are not
...
even though pingvincoin-config.h indicates that a faster feature is available and should be used.
...
#include <config/pingvincoin-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
^^^
None of the files use a symbol declared in the pingvincoin-config.h header. However, they are including
