...
from test_framework.test_framework import PingvincoinTestFramework
from test_framework.test_node import ErrorMatch
from test_framework import util


class ConfArgsTest(PingvincoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in pingvincoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "pingvincoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'pingvincoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "pingvincoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by pingvincoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a pingvincoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.pingvincoinconf.read_text()
        conf_path = default_datadir / "pingvincoin.conf"
        ...
        # (conf_path, not node.pingvincoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a pingvincoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "pingvincoin.conf" file which is ignored, because a '
        ...
        # the same pingvincoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/pingvincoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when pingvincoin.conf in the default data directory sets another datadir '
                      'and it contains a different pingvincoin.conf file that would be ignored')
        ...
        # pingvincoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "pingvincoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "pingvincoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/pingvincoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "pingvincoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "pingvincoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_pingvincoin.conf"
