#!/usr/bin/env python3
# Copyright (c) 2014-present The Pingvincoin Core developers
...
TMPDIR_PREFIX = "pingvincoin_func_test_"
...
class PingvincoinTestMetaClass(type):
    """Metaclass for PingvincoinTestFramework.

    Ensures that any attempt to register a subclass of `PingvincoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'PingvincoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("PingvincoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("PingvincoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class PingvincoinTestFramework(metaclass=PingvincoinTestMetaClass):
    """Base class for a pingvincoin test script.

    Individual pingvincoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use pingvincoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "pingvincoind": ("pingvincoind", "BITCOIND"),
            "pingvincoin-cli": ("pingvincoincli", "BITCOINCLI"),
            "pingvincoin-util": ("pingvincoinutil", "BITCOINUTIL"),
            "pingvincoin-wallet": ("pingvincoinwallet", "BITCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: pingvincoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("PingvincoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'pingvincoind', self.options.pingvincoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'pingvincoin-cli', self.options.pingvincoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            test_node_i = TestNode(
                ...
                pingvincoind=binary[i],
                pingvincoin_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a pingvincoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple pingvincoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the pingvincoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as pingvincoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    pingvincoind=self.options.pingvincoind,
                    pingvincoin_cli=self.options.pingvincoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in pingvincoin.conf
    ...
    def skip_if_no_pingvincoind_tracepoints(self):
        """Skip the running test if pingvincoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("pingvincoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_pingvincoind_zmq(self):
        """Skip the running test if pingvincoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("pingvincoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if pingvincoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("pingvincoin-wallet has not been compiled")

    def skip_if_no_pingvincoin_util(self):
        """Skip the running test if pingvincoin-util has not been compiled."""
        if not self.is_pingvincoin_util_compiled():
            raise SkipTest("pingvincoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if pingvincoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("pingvincoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether pingvincoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether pingvincoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_pingvincoin_util_compiled(self):
        """Checks whether pingvincoin-util was compiled."""
