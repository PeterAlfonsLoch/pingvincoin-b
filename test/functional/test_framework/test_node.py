#!/usr/bin/env python3
# Copyright (c) 2017-2022 The Pingvincoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Class for pingvincoind node under test"""
...
class TestNode():
    """A class for representing a pingvincoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, pingvincoind, pingvincoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the pingvincoin.conf file.
        # This means that starting a pingvincoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(pingvincoin_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any pingvincoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then pingvincoind would bind P2P ports on
        ...
        # pingvincoin.conf. To avoid collisions on 127.0.0.1:18445, change it to
        # 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time pingvincoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by pingvincoind, and
        ...
        self.running = True
        self.log.debug("pingvincoind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the pingvincoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'pingvincoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; pingvincoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to pingvincoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; pingvincoind is still starting
    ...
    def replace_in_config(self, replacements):
        ...
        The substitutions are passed as a list of search-replace-tuples, e.g.
            [("old", "new"), ("foo", "bar"), ...]
        """
        with open(self.pingvincoinconf, 'r', encoding='utf8') as conf:
            conf_data = conf.read()
        for replacement in replacements:
            assert_equal(len(replacement), 2)
            old, new = replacement[0], replacement[1]
            conf_data = conf_data.replace(old, new)
        with open(self.pingvincoinconf, 'w', encoding='utf8') as conf:
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into pingvincoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to pingvincoind
        expected_msg: regex that stderr should match when pingvincoind fails

        Will throw if pingvincoind starts without an error.
        Will throw if an expected_msg is provided and it does not match pingvincoind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'pingvincoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'pingvincoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to pingvincoin-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.pingvincoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with pingvincoin-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run pingvincoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running pingvincoin-cli {}".format(p_args[2:]))
