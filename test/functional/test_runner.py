#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Pingvincoin Core developers
...
`test/functional/test_framework/test_framework.py:PingvincoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_pingvincoin_cli.py --legacy-wallet',
    'interface_pingvincoin_cli.py --descriptors',
...
def main():
    ...
    enable_pingvincoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_pingvincoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if pingvincoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "pingvincoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a pingvincoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Pingvincoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/pingvincoin/pingvincoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `pingvincoin-cli help` (`rpc_interface.txt`).
