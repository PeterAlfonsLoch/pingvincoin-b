#!/usr/bin/env python3
# Copyright (c) 2021-present The Pingvincoin Core developers
...
to a hash that has been compiled into pingvincoind.
...
class AssumeutxoTest(PingvincoinTestFramework):
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test pingvincoind should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test pingvincoind should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test pingvincoind should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
