# Bootstrappable Pingvincoin Core Builds

This directory contains the files necessary to perform bootstrappable Pingvincoin
Core builds.
...
To build Pingvincoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Pingvincoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/pingvincoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/pingvincoin-core/guix.sigs).
