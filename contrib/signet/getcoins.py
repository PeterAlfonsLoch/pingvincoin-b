#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Pingvincoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing pingvincoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='pingvincoin-cli', help='pingvincoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Pingvincoin address to which the faucet should send')
...
parser.add_argument('pingvincoin_cli_args', nargs='*', help='Arguments to pass on to pingvincoin-cli (default: -signet)')

args = parser.parse_args()

if args.pingvincoin_cli_args == []:
    args.pingvincoin_cli_args = ['-signet']


def pingvincoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.pingvincoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = pingvincoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = pingvincoin_cli(['getnewaddress', 'faucet', 'bech32'])
