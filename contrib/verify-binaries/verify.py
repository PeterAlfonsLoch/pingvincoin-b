...
"""Script for verifying Pingvincoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from pingvincoincore.org and pingvincoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/pingvincoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://pingvincoincore.org"
HOST2 = "https://pingvincoin.org"
VERSIONPREFIX = "pingvincoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Pingvincoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"pingvincoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by pingvincoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the pingvincoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes pingvincoin.org lags behind pingvincoincore.org.)')
    )
