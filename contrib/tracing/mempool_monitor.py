...
""" Example logging Pingvincoin Core mempool events using the mempool:added,
...
def main(pingvincoind_path):
    pingvincoind_with_usdts = USDT(path=str(pingvincoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    pingvincoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    pingvincoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    pingvincoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    pingvincoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[pingvincoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/pingvincoind")
        exit(1)
