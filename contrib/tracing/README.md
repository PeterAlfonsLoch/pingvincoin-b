...
Tracing (USDT) support for Pingvincoin Core on Linux using. For more information on
USDT support in Pingvincoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `pingvincoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `pingvincoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Pingvincoin Core USDT documentation] on how to list available tracepoints in your
`pingvincoind` binary.

[Pingvincoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/pingvincoind
```
...
Pingvincoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/pingvincoind
```
...
In a different terminal, starting Pingvincoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/pingvincoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/pingvincoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/pingvincoind
```
