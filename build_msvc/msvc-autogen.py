...
libs = [
    ...
    'libpingvincoin_crypto',
    'libpingvincoin_node',
    'libpingvincoin_util',
    'libpingvincoin_wallet_tool',
...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Pingvincoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/pingvincoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/pingvincoin_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='Pingvincoin-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/pingvincoin_config.h'), os.path.join(SOURCE_DIR, 'config/pingvincoin-config.h'))
