...
{
    // If Qt is used, parameters/pingvincoin.conf are parsed in qt/pingvincoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see pingvincoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  pingvincoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for pingvincoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect pingvincoind signal handlers
    noui_connect();
