// Copyright (c) 2011-2022 The Pingvincoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <config/pingvincoin-config.h> // IWYU pragma: keep

#include <qt/pingvincoingui.h>

#include <qt/pingvincoinunits.h>
...
const std::string PingvincoinGUI::DEFAULT_UIPLATFORM =
...
PingvincoinGUI::PingvincoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &PingvincoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &PingvincoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &PingvincoinGUI::showModalOverlay);
...
void PingvincoinGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &PingvincoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &PingvincoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &PingvincoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Pingvincoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Pingvincoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a pingvincoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Pingvincoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &PingvincoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &PingvincoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &PingvincoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &PingvincoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &PingvincoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &PingvincoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &PingvincoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &PingvincoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &PingvincoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &PingvincoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &PingvincoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &PingvincoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &PingvincoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &PingvincoinGUI::showDebugWindow);
}

void PingvincoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void PingvincoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &PingvincoinGUI::setCurrentWalletBySelectorIndex);
...
void PingvincoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &PingvincoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &PingvincoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &PingvincoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &PingvincoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void PingvincoinGUI::enableHistoryAction(bool privacy)
{
...
void PingvincoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &PingvincoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &PingvincoinGUI::removeWallet);
...
WalletController* PingvincoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void PingvincoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &PingvincoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &PingvincoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &PingvincoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &PingvincoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &PingvincoinGUI::incomingTransaction);
    connect(this, &PingvincoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void PingvincoinGUI::removeWallet(WalletModel* walletModel)
{
...
void PingvincoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void PingvincoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void PingvincoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PingvincoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void PingvincoinGUI::createTrayIcon()
{
...
void PingvincoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &PingvincoinGUI::toggleHidden);
...
void PingvincoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void PingvincoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void PingvincoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void PingvincoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void PingvincoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void PingvincoinGUI::openClicked()
{
...
void PingvincoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PingvincoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void PingvincoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void PingvincoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PingvincoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PingvincoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void PingvincoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void PingvincoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Pingvincoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void PingvincoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PingvincoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void PingvincoinGUI::updateHeadersSyncProgressLabel()
{
...
void PingvincoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void PingvincoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &PingvincoinGUI::quitRequested);
...
void PingvincoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void PingvincoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &PingvincoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void PingvincoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void PingvincoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &PingvincoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &PingvincoinGUI::show);
                e->ignore();
            }
        }
    }
...
void PingvincoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PingvincoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PingvincoinGUI::incomingTransaction(const QString& date, PingvincoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(PingvincoinUnits::formatWithUnit(unit, amount, true));
...
void PingvincoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PingvincoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PingvincoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PingvincoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PingvincoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void PingvincoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PingvincoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void PingvincoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void PingvincoinGUI::updateWindowTitle()
{
...
void PingvincoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PingvincoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PingvincoinGUI::detectShutdown()
{
...
void PingvincoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void PingvincoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(PingvincoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = PingvincoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void PingvincoinGUI::subscribeToCoreSignals()
{
...
void PingvincoinGUI::unsubscribeFromCoreSignals()
{
...
bool PingvincoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<PingvincoinUnit> units = PingvincoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PingvincoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, PingvincoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const PingvincoinUnit u : PingvincoinUnits::availableUnits()) {
        menu->addAction(PingvincoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(PingvincoinUnit newUnits)
{
    setText(PingvincoinUnits::longName(newUnits));
}
