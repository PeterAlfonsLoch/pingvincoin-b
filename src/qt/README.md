This directory contains the source code for the Pingvincoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### pingvincoingui.(h/cpp)

- Represents the main window of the Pingvincoin UI.
...
- ClientModel is used by the main application `pingvincoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a pingvincoin:... link from a browser).
...
* UI elements like PingvincoinAmountField, which inherit from QWidget.
* `pingvincoinstrings.cpp`: automatically generated
* `pingvincoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/pingvincoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `pingvincoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `pingvincoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `pingvincoin-qt`
9. You're all set! Start developing, building, and debugging the Pingvincoin Core GUI
