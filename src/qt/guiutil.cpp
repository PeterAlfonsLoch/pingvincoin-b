...
#include <qt/pingvincoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new PingvincoinAddressEntryValidator(parent));
    widget->setCheckValidator(new PingvincoinAddressCheckValidator(parent));
}
...
bool parsePingvincoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!PingvincoinUnits::parse(PingvincoinUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parsePingvincoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsePingvincoinURI(uriInstance, out);
}

QString formatPingvincoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("pingvincoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PingvincoinUnits::format(PingvincoinUnit::BTC, info.amount, false, PingvincoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPingvincoinConf()
{
    ...
    /* Open pingvincoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Pingvincoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Pingvincoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "pingvincoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("pingvincoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a pingvincoin.desktop file to the autostart directory:
