...
/** Call node.updateRwSetting() with Pingvincoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Pingvincoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/pingvincoin/pingvincoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because pingvincoin 22.x will not try to read these.
...
//! Convert enabled/size values to pingvincoin -prune setting.
...
//! Get pruning enabled value to show in GUI from pingvincoin -prune setting.
...
//! Get pruning size value to show in GUI from pingvincoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayPingvincoinUnit")) {
        settings.setValue("DisplayPingvincoinUnit", QVariant::fromValue(PingvincoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayPingvincoinUnit");
    if (unit.canConvert<PingvincoinUnit>()) {
        m_display_pingvincoin_unit = unit.value<PingvincoinUnit>();
    } else {
        m_display_pingvincoin_unit = PingvincoinUnit::BTC;
        settings.setValue("DisplayPingvincoinUnit", QVariant::fromValue(m_display_pingvincoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if pingvincoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_pingvincoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<PingvincoinUnit>() == m_display_pingvincoin_unit) return;
    m_display_pingvincoin_unit = new_unit.value<PingvincoinUnit>();
    QSettings settings;
    settings.setValue("DisplayPingvincoinUnit", QVariant::fromValue(m_display_pingvincoin_unit));
    Q_EMIT displayUnitChanged(m_display_pingvincoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/pingvincoin/pingvincoin/pull/8273
    ...
    // (https://github.com/pingvincoin-core/gui/issues/567).
