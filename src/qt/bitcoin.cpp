...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<PingvincoinUnit>("PingvincoinUnit");
#else
    qRegisterMetaType<PingvincoinUnit>("PingvincoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. pingvincoin_de.qm (shortcut "de" needs to be defined in pingvincoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. pingvincoin_de_DE.qm (shortcut "de_DE" needs to be defined in pingvincoin.qrc)
...
PingvincoinApplication::PingvincoinApplication()
...
void PingvincoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the PingvincoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", PingvincoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void PingvincoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool PingvincoinApplication::createOptionsModel(bool resetSettings)
{
...
void PingvincoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new PingvincoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &PingvincoinGUI::quitRequested, this, &PingvincoinApplication::requestShutdown);
...
void PingvincoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void PingvincoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool PingvincoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void PingvincoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &PingvincoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &PingvincoinApplication::handleRunawayException);
    connect(this, &PingvincoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &PingvincoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void PingvincoinApplication::parameterSetup()
{
...
void PingvincoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void PingvincoinApplication::requestInitialize()
{
...
void PingvincoinApplication::requestShutdown()
{
...
void PingvincoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // pingvincoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PingvincoinGUI::handlePaymentRequest);
            connect(window, &PingvincoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void PingvincoinApplication::handleRunawayException(const QString &message)
{
...
void PingvincoinApplication::handleNonFatalException(const QString& message)
{
...
WId PingvincoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool PingvincoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", PingvincoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(pingvincoin);
    Q_INIT_RESOURCE(pingvincoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see pingvincoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see pingvincoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse pingvincoin.conf, determine network, switch to network specific
    ...
    // pingvincoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
