...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //pingvincoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Pingvincoin
    //   new: C:\Users\Username\AppData\Local\Pingvincoin
    // macOS: ~/Library/Application Support/Pingvincoin
    // Unix-like: ~/.pingvincoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Pingvincoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Pingvincoin";
#else
    fs::path pathRet;
